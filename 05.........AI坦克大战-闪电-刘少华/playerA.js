...
window.playerA = new (class PlayerControl {
    ...
    constructor(type) {
        ...
        this.svNoStopDirections = [this.DIRECTION.DOWN, this.DIRECTION.UP,  this.DIRECTION.LEFT, this.DIRECTION.RIGHT];
        this.loopCount = 0;
        // 危险距离
        ...
        this.SUPER_ATTACK_DISTANCE = 2.5 * this.TANK_WIDTH;
        this.SUPER_ATTACH_MAX_DISTANCE = 8 * this.TANK_WIDTH;
        this.ATTACK_DISTANCE = 6 * this.TANK_WIDTH;
        this.inSuperAttackMode = false;
        this.minAttackDistance = 10;
        this.ATTACK_MIN_DISTANCE_CONST = 198;
        this.ATTACK_MIN_DISTANCE = this.ATTACK_MIN_DISTANCE_CONST;
        this.lastEnemyDirection = this.DIRECTION.STOP;
        this.fireIgnoreDirection = false;
    }
    ...
    land() {
        ...
        // PlayerA/PlayerB
        this.myBullets = aMyBulletCount1;
        this.enemyBullets = aMyBulletCount2;
        this.myCollideNum = player1CollideNum;
        this.enemyCollideNum = player2CollideNum;
        ...
        let shouldFire = array[2];

        let finalDirection = direction;
        if (targetTank === this.enemyTank) {
            let dx = this.svMyTank.X - targetTank.X;
            let dy = this.svMyTank.Y - targetTank.Y;
            if (!this.inMustDieArea2(dx, dy)) {
                // 必死局面下不躲避
                finalDirection = this.CheckIfIndangerAndAvoid(direction);
            } else {
                this.fireIgnoreDirection = true;
                this.log("只管打");
            }
        } else {
            finalDirection = this.CheckIfIndangerAndAvoid(direction);
        ...
        if (finalDirection !== direction) {
            // 最终方向与设定方向不一致，退出超级攻击模式
            this.inSuperAttackMode = false;
            this.superAttackDirections = [];
            this.firstAttackLoopCount = 0;
            // 舍弃traceAndFire阶段产生的是否开火
            shouldFire = false;
        }

        if (this.isCounterStrike() && this.targetTankInfo[2] === this.enemyTank && this.allEnemyTank.length <= 10) {
            if (this.allEnemyTank.length === 1 && this.isBlockByMetal(this.svMyTank, targetTank)) {
                // 只有对手方坦克且有障碍物阻挡时设置为8
                this.DANGER_STEP = 8;
            } else if (this.inSuperAttackMode) {
                this.DANGER_STEP = 15;
            } else {
                this.DANGER_STEP = 18;
            }
        ...
        this.log("DANGER_STEP", this.DANGER_STEP);
        
        this.log("positions: ", this.svMyTank.X, this.svMyTank.Y, targetTank.X, targetTank.Y);

        // PlayerA/PlayerB
        if (finalDirection === this.DIRECTION.RIGHT && this.loopCount - this.lastEnemyDirectionFire >= 80 && this.allEnemyTank.length >= 1 && targetTank !== this.enemyTank) {
            shouldFire = true;
        ...
        if (shouldFire && this.shouldFireLoopTimes.length < 5) {
            ...
            let shouldFireLoopTime = 0;
            if (this.fireIgnoreDirection) {
                shouldFireLoopTime = this.loopCount;
                this.fireIgnoreDirection = false;
            } else {
                shouldFireLoopTime = finalDirection === this.svMyTank.direction ? this.loopCount : this.loopCount + 1;
            }
            if (this.shouldFireLoopTimes.length > 0) {
                // 防止连续两个子弹发出的情况
                // TODO SCJ
                // if (this.shouldFireLoopTimes[this.shouldFireLoopTimes.length-1] !== this.loopCount) {
        ...
        this.log("finalDirection", finalDirection);

        this.SVMove(finalDirection);

        this.log("shouldFireLoopTimes: ", this.shouldFireLoopTimes);
        ...
        this.log("targetTank.direction", this.SVGetDirectionStr(targetTank.direction));

        if (targetTank === this.enemyTank) {
            this.lastEnemyDirection = targetTank.direction;
        }
    ...
    shouldFireOnDirection(direction) {
        ...
        if (targetTank === this.enemyTank) {
            ...
            let dx = this.svMyTank.X - targetTank.X;
            let dy = this.svMyTank.Y - targetTank.Y;
            if (this.myBullets.length >= 3) {
                if (this.inMustDieArea2(dx, dy)) {
                    this.log("in MustDieArea");
                    return true;
                } else {
                    this.log("NOT in MustDieArea");
                    // 非必杀模式也不是超级攻击模式，则不能多玩3个子弹（留两个子弹在必杀时使用）
                    if (!this.inSuperAttackMode) {
                        return false;
                    } else {
                        // 其他情况走下面
                    }
        ...
        this.error("nearestDistance", nearestDistance);
        if (nearestDistance <= canvas.width*3/4) {
            ...
            if (myBulletCount < tankCount+spareNum) {
                return true;
            }
        } else {
            let maxBulletsCount = 2;
            if (targetTank === this.enemyTank) {
                let dx = this.svMyTank.X - targetTank.X;
                let dy = this.svMyTank.Y - targetTank.Y;
                if (this.inMustDieArea2(dx, dy)) {
            ...
            if (myBulletCount < maxBulletsCount) {
                return true;
            }
        }
        return false;
            ...
            switch (direction) {
                case this.DIRECTION.UP:
                    if (Math.abs(this.svMyTank.X - enemy.X) < this.MIN_FIRE_OFFSET && enemy.Y < this.svMyTank.Y) {
                        ...
                        if (enemy.Y > maxYOnUp) {
                            maxYOnUp = enemy.Y;
                            nearestDistance = this.svMyTank.Y - maxYOnUp;
                        }
                    }
                    break;
                case this.DIRECTION.RIGHT:
                    if (Math.abs(this.svMyTank.Y - enemy.Y) < this.MIN_FIRE_OFFSET && enemy.X > this.svMyTank.X) {
                        ...
                        if (enemy.X < minXOnRight) {
                            minXOnRight = enemy.X;
                            nearestDistance = minXOnRight - this.svMyTank.X;
                        }
                    }
                    break;
                case this.DIRECTION.DOWN:
                    if (Math.abs(this.svMyTank.X - enemy.X) < this.MIN_FIRE_OFFSET && enemy.Y > this.svMyTank.Y) {
                        ...
                        if (enemy.Y < minYOnDown) {
                            minYOnDown = enemy.Y;
                            nearestDistance = minYOnDown - this.svMyTank.Y;
                        }
                    }
                    break;
                case this.DIRECTION.LEFT:
                    if (Math.abs(this.svMyTank.Y - enemy.Y) < this.MIN_FIRE_OFFSET && enemy.X < this.svMyTank.X) {
                        ...
                        if (enemy.X > maxXOnLeft) {
                            maxXOnLeft = enemy.X;
                            nearestDistance = this.svMyTank.X - maxXOnLeft;
                        }
                    }
                    break;
            }
    ...
    findRecommendPath(inputDirections, currentScore) {
        ...
        if (recommendPath !== undefined) {
            ...
            let array = this.deduceForOneDirection(this.dangerBullets);
    ...
    deduceIfCrashForImaginBullets(bullets) {
        for(let k=0;k<this.svNoStopDirections.length;k++) {
            let direction = this.svNoStopDirections[k];
            let crash = false;
            for(let i=1;i<=10 && !crash; i++) {
                let score = this.scoreOfNSameDirections(direction, i, bullets, this.targetTankInfo[2]);
                if (score > 0) {
        ...
        // 没有最好的分数的情况下，返回true,即所有方向都会相撞
        return true;
    }

    deduceForOneDirection(bullets) {
        let highestScore = this.WORST_SCORE;
        ...
        for(let k=0;k<this.svNoStopDirections.length;k++) {
            ...
            for(let i=1;i<=this.MAX_DEDUCE_COUNT_FOR_ONE_DIR && !crash; i++) {
                let score = this.scoreOfNSameDirections(direction, i, bullets, this.svMyTank);
    ...
    // 往同一个方向推演N步后分析局面得分
    scoreOfNSameDirections(direction, step, bullets, tank) {
        let imaginaryTank = this.copyOfTank(tank);
    ...
    analyzeCurrentSituation(imaginaryTank, bullets, step, direction) {
        ...
        if (allBulletsSameDirection && !isBestScore && this.targetTankInfo[2] !== this.enemyTank) {
    ...
    traceAndFireV2() {
        ...
        if (targetTank === this.enemyTank) {
            ...
            if (this.MIN_FIRE_INTERVAL >= 119) {
                this.MIN_FIRE_INTERVAL = 119;
            }
            if (this.inCautionArea(dx, dy)) {
                // 关键时刻，不能吝惜子弹
                this.MIN_FIRE_INTERVAL = 0;
            }
            this.MIN_FIRE_OFFSET = (this.TANK_WIDTH + this.BULLET_WIDTH)/2;
            ...
            this.DANGER_DISTANCE = 2.3 * this.TANK_WIDTH;
            this.BEST_FIRE_DISTANCE = 3.1 * this.TANK_WIDTH;
        ...
        if (this.myCollideNum > 10) {
            ...
            // 有可绕障碍并且有足够的逃跑距离
            if (bestMetal !== undefined && distance > 2.5 * this.TANK_WIDTH) {
                // 如果对手方坦克已死，则不执行逃跑策略
                if (this.enemyTank !== undefined) {
                    finalDirection = this.escape4Enemy(bestMetal);
                    // 专注于逃跑，不管其他， 如果无法逃跑再执行其他策略
                    return [finalDirection, targetTank, shouldFire];
                }
            } else {
                // 考虑找不到的情况: 击杀
                if (targetTank === this.enemyTank) {
                    let array = this.approachAndFire(dx, dy);
                    finalDirection = array[0];
                    shouldFire = array[1];
                    return [finalDirection, targetTank, shouldFire];
                }
        ...
        if (this.inDangerArea(this.svMyTank, targetTank)) {
            // 远离（根据是否靠近边界）
            finalDirection = this.escape4AI(dx, dy);
    ...
    approachAndFire(dx, dy) {
        this.log("dx, dy", dx, dy);
        if (this.inSuperAttackArea(dx, dy)) {
            // 进入超级攻击模式
            this.inSuperAttackMode = true;
        } 
        if (this.inSuperAttackMode) {
            this.log("in super area");
            return this.superAttack(dx, dy);
        } if (Math.abs(dx) < this.minAttackDistance && Math.abs(dy) < this.minAttackDistance) {
            this.log("in stop and wait area");
            return [this.DIRECTION.STOP, false];
        } else if (this.inLimitSafeArea(dx, dy)) {
            this.log("in LimitSafeArea area");
            return this.veryCautionAttack(dx, dy);
        } else if (this.inAfterStepOneArea(dx, dy)) {
            this.log("in afterStepOne area");
            return this.afterStepOne(dx, dy);
        } else if (this.inAttackArea(dx, dy) || this.inCautionArea(dx, dy)) {
        // } else if (this.inAttackArea(dx, dy)) {
            this.log("in attack/caution/safe area");
            return [this.attack(dx, dy), false];
        } else {
            this.log("in find path area");
            this.DANGER_DISTANCE = 0;
            this.BEST_FIRE_DISTANCE = this.TANK_WIDTH;
            return [this.findBestFireArea(this.targetTankInfo[2]), false];
        }
    }
    ...
    getPositionScoreOfFireArea(point) {
        ...
        if (this.targetTankInfo[2] === this.enemyTank) {
            return (-distance/2 - this.getDistanceOfPoints({X:canvas.width/2, Y:canvas.height/2}, point))/2;
        }
    ...
    move2Point(targetPoint) {
        ...
        if (targetTank === this.enemyTank) {
            ...
            if (Math.abs(dx) > canvas.width/4 && targetTank !== this.enemyTank) {
    ...
    inAfterStepOneArea(dx, dy) {
        let absDx = Math.abs(dx);
        let absDy = Math.abs(dy);
        if (absDy < absDy) {
            if (absDx < 30 && absDy < 37) {
                return true;
            }
        } else {
            if (absDx < 37 && absDy < 30) {
                return true;
            }
        }
    ...
    inLimitSafeArea(dx, dy) {
        let absDx = Math.abs(dx);
        let absDy = Math.abs(dy);
        if (absDx < 37 && absDx >= 30 && absDy < 37 && absDy < 37 >= 30) {
            return true;
        }
        return false;
    }
    ...
    inCautionArea(dx, dy) {
        ...
        let absDy = Math.abs(dy);
        if (absDx < absDy) {
            if (absDy < 1.5 * this.TANK_WIDTH) {
                return true;
            }
        } else {
            if (absDx < 1.5 * this.TANK_WIDTH) {
                return true;
            }
        }
        return false;
    ...
    inSuperAttackArea(dx, dy) {
        let myBulletCount = this.myBullets.length;
        this.ATTACK_MIN_DISTANCE = this.ATTACK_MIN_DISTANCE_CONST - myBulletCount * 42
        let targetTank = this.targetTankInfo[2];
        let minDistanceOfTarget = this.minDistanceOfBound(targetTank);
        let superMinFireOffset = this.TANK_WIDTH;
        if (minDistanceOfTarget <= this.ATTACK_MIN_DISTANCE-2) {
            superMinFireOffset = superMinFireOffset + (this.ATTACK_MIN_DISTANCE-2-minDistanceOfTarget);
        }
        if ((Math.abs(dx) < superMinFireOffset && Math.abs(dy) >= this.SUPER_ATTACK_DISTANCE && Math.abs(dy) <= this.SUPER_ATTACH_MAX_DISTANCE) || (Math.abs(dy) < superMinFireOffset && Math.abs(dx) >= this.SUPER_ATTACK_DISTANCE && Math.abs(dx) <= this.SUPER_ATTACH_MAX_DISTANCE)) {
            if (this.isBlockByMetal(this.svMyTank, this.targetTankInfo[2])) {
                return false;
            }
            ...
            let minDistanceOfMyAndEnemy = 20;
            let minDistanceOfSuperAttack = this.ATTACK_MIN_DISTANCE;
            let distanceOfSuperAttack = minDistanceOfSuperAttack + this.TANK_WIDTH/2;
            let centerOfMyTank = {X:this.svMyTank.X+this.TANK_WIDTH/2, Y:this.svMyTank.Y+this.TANK_WIDTH/2};
            let centerOfTargetTank = {X:targetTank.X+this.TANK_WIDTH/2, Y:targetTank.Y+this.TANK_WIDTH/2};
            if (centerOfMyTank.Y - centerOfTargetTank.Y >= minDistanceOfMyAndEnemy && centerOfMyTank.Y < distanceOfSuperAttack) {
                return true;
            }
            if (centerOfMyTank.X - centerOfTargetTank.X >= minDistanceOfMyAndEnemy && centerOfMyTank.X < distanceOfSuperAttack) {
                return true;
            }
            if (centerOfTargetTank.Y - centerOfMyTank.Y >= minDistanceOfMyAndEnemy && centerOfMyTank.Y > canvas.height - distanceOfSuperAttack) {
                return true;
            }
            if (centerOfTargetTank.X - centerOfMyTank.X >= minDistanceOfMyAndEnemy && centerOfMyTank.X > canvas.width - distanceOfSuperAttack) {
                return true;
            }
            ...
            for(let i=0;i<ametal.length; i++) {
                ...
                let deltaX = Math.abs(centerOfTargetTank.X - centerOfMetal.X);
                let deltaY = Math.abs(centerOfTargetTank.Y - centerOfMetal.Y);
                let deltaXOfMine = Math.abs(centerOfMyTank.X - centerOfMetal.X);
                let deltaYOfMine = Math.abs(centerOfMyTank.Y - centerOfMetal.Y);
                if (deltaY < (this.METAL_WIDTH+this.TANK_WIDTH)/2 && deltaX < (this.TANK_WIDTH+this.METAL_WIDTH)/2+minDistanceOfSuperAttack && deltaXOfMine > deltaX && Math.abs(dx) < superMinFireOffset) {
                    return true;
                }
                if (deltaX < (this.METAL_WIDTH+this.TANK_WIDTH)/2 && deltaY < (this.TANK_WIDTH+this.METAL_WIDTH)/2+minDistanceOfSuperAttack && deltaYOfMine > deltaY && Math.abs(dy) < superMinFireOffset) {
                    return true;
                }
            }
    ...
    minDistanceOfBound(point) {
        // 计算点到边界的最小距离
        let minDx = Math.min(point.X, canvas.width-point.X-this.TANK_WIDTH);
        let minDy = Math.min(point.Y, canvas.height-point.Y-this.TANK_WIDTH);
        let minDelta = Math.min(minDx, minDy);
        return minDelta;
    }

    inSaveBulletsArea(dx, dy) {
        if (Math.abs(dx) < 57 + this.TANK_WIDTH || Math.abs(dy) < 57 + this.TANK_WIDTH) {
            return true;
        }
        return false;
    }

    inMustDieArea2(dx, dy) {
        let absDx = Math.abs(dx);
        let absDy = Math.abs(dy);
        if (absDx < 23 && absDy < 23) {
            return true;
        }
        let dMax = Math.max(absDx, absDy);
        let dMin = Math.min(absDx, absDy);
        if (dMax < 37 && dMin < 16) {
            return true;
        }
        if (dMax >= 57+this.TANK_WIDTH) {
            // 根据坦克不同方向，最小距离要么为56，要么为49
            return false;
        }

        let imaginBullet = {};

        switch(this.svMyTank.direction) {
            case this.DIRECTION.LEFT:
                imaginBullet = new Bullet('imagin1', 10, 1, this.DIRECTION.LEFT, this.svMyTank.X-this.BULLET_WIDTH/2, this.svMyTank.Y+this.TANK_WIDTH/2);
                break;
            case this.DIRECTION.UP:
                imaginBullet = new Bullet('imagin2', 10, 1, this.DIRECTION.UP, this.svMyTank.X+this.TANK_WIDTH/2, this.svMyTank.Y-this.BULLET_WIDTH/2);
                break;
            case this.DIRECTION.RIGHT:
                imaginBullet = new Bullet('imagin3', 10, 1, this.DIRECTION.RIGHT, this.svMyTank.X+this.TANK_WIDTH+this.BULLET_WIDTH/2, this.svMyTank.Y+this.TANK_WIDTH/2);
                break;        
            case this.DIRECTION.DOWN:
                imaginBullet = new Bullet('imagin4', 10, 1, this.DIRECTION.DOWN, this.svMyTank.X + this.TANK_WIDTH/2, this.svMyTank.Y+this.TANK_WIDTH+this.BULLET_WIDTH/2);
                break;
        }

        imaginBullet.width = this.BULLET_WIDTH;

        let imaginBullets = [imaginBullet];

        return this.deduceIfCrashForImaginBullets(imaginBullets);
    }
    ...
    inMustDieArea(dx, dy) {
        ...
        // 左、上、右、下四个子弹各自的offset(x,y)
        let offsets = [];
        switch(this.svMyTank.direction) {
            case this.DIRECTION.LEFT:
                offsets = [[0,0],[0,-7],[7,0],[0,7]];
                break;
            case this.DIRECTION.UP:
                offsets = [[-7,0],[0,0],[7,0],[0,7]];
                break;
            case this.DIRECTION.RIGHT:
                offsets = [[-7,0],[0,-7],[0,0],[0,7]];
                break;        
            case this.DIRECTION.DOWN:
                offsets = [[-7,0],[0,-7],[7,0],[0,7]];
                break;
        }

        let imaginBulletLeft = new Bullet('imagin1', 10, 1, this.DIRECTION.LEFT, this.svMyTank.X-this.BULLET_WIDTH/2, this.svMyTank.Y+this.TANK_WIDTH/2);
        let imaginBulletTop = new Bullet('imagin2', 10, 1, this.DIRECTION.UP, this.svMyTank.X+this.TANK_WIDTH/2, this.svMyTank.Y-this.BULLET_WIDTH/2);
        let imaginBulletRight = new Bullet('imagin3', 10, 1, this.DIRECTION.RIGHT, this.svMyTank.X+this.TANK_WIDTH+this.BULLET_WIDTH/2, this.svMyTank.Y+this.TANK_WIDTH/2);
        let imaginBulletBottom = new Bullet('imagin4', 10, 1, this.DIRECTION.DOWN, this.svMyTank.X + this.TANK_WIDTH/2, this.svMyTank.Y+this.TANK_WIDTH+this.BULLET_WIDTH/2);
        let imaginBullets = [imaginBulletLeft, imaginBulletTop, imaginBulletRight, imaginBulletBottom];
        for(let i=0; i<imaginBullets.length; i++) {
            let imaginBullet = imaginBullets[i];
            imaginBullet.X += offsets[i][0];
            imaginBullet.Y += offsets[i][1];
            imaginBullet.width = this.BULLET_WIDTH;
        }
    ...
    findBestMetal2Escape() {
        ...
        for(let i=0; i<ametal.length;i++) {
            ...
            let nearestVertexOfMyTank = this.getNearestVertexOfTanks(this.svMyTank, {X:targetTank.X+this.TANK_WIDTH/2, Y:targetTank.Y+this.TANK_WIDTH/2});
            let nearestVertexOfTarget = this.getNearestVertexOfTanks(targetTank, {X:this.svMyTank.X+this.TANK_WIDTH/2, Y:this.svMyTank.Y+this.TANK_WIDTH/2});

            let dx1 = nearestVertexOfMyTank.X - nearestVertexOfTarget.X;
            let dx2 = nearestVertexOfTank.X - nearestVertex.X;
            let dy1 = nearestVertexOfMyTank.Y - nearestVertexOfTarget.Y;
    ...
    getNearestVertexOfTanks(tank, centerPoint) {
        let nearestVertex = {};
        ...
        for(let i=0;i<vertexs.length;i++) {
            ...
            let distance = this.getDistanceOfPoints({X:centerPoint.X, Y:centerPoint.Y}, vertex);
    ...
    superAttack(dx, dy) {
        if (this.superAttackDirections.length > 0 && this.firstAttackLoopCount > 0) {
            let index = this.loopCount - this.firstAttackLoopCount;
            if (index < this.superAttackDirections.length) {
                return this.superAttackDirections[index];
            } else {
                this.superAttackDirections = [];
                this.firstAttackLoopCount = 0;
                this.inSuperAttackMode = false;
        ...
        let absDX = Math.abs(dx);
        let absDy = Math.abs(dy);
        let firstDirection = this.DIRECTION.STOP;
        let afterDirections = this.DIRECTION.STOP;
        // 重合，开火
        if (absDX > absDy) {
            if (dx > 0) {
                // 我方在右边
                firstDirection = this.DIRECTION.LEFT;
            } else {
                // 我方在左边
                firstDirection = this.DIRECTION.RIGHT;
            }
            if (dy > 0) {
                afterDirections = this.DIRECTION.UP;
            } else {
                afterDirections = this.DIRECTION.DOWN;
            }
        } else {
            if (dy > 0) {
                firstDirection = this.DIRECTION.UP;
            } else {
                firstDirection = this.DIRECTION.DOWN;
            }
            if (dx > 0) {
                afterDirections = this.DIRECTION.LEFT;
            } else {
                afterDirections = this.DIRECTION.RIGHT;
            }
        }
        var breaked = false;
        for(let j=0;j<5 && !breaked;j++) {
            this.superAttackDirections.push([firstDirection, true]);
            for(let i=0; i<6; i++) {
                var copyOfMyTank = this.copyOfTank(this.svMyTank);
                copyOfMyTank.direction = afterDirections;
                var step = j * 6 + i;
                var tankAfterMove = this.imagineMove(step, copyOfMyTank, true);
                if ((afterDirections === this.DIRECTION.UP && tankAfterMove.Y <= 0)
                    || (afterDirections === this.DIRECTION.DOWN && tankAfterMove.Y >= canvas.height-this.TANK_WIDTH)
                    || (afterDirections === this.DIRECTION.LEFT && tankAfterMove.X <= 0)
                    || (afterDirections === this.DIRECTION.RIGHT && tankAfterMove.X >= canvas.width-this.TANK_WIDTH)) {
                        breaked = true;
                        break;
                }
                this.superAttackDirections.push([afterDirections, false]);    
            }
        }
        this.firstAttackLoopCount = this.loopCount;
    ...
    attack(dx, dy) {
        ...
        let targetTank = this.targetTankInfo[2];
        if (absDX > absDy) {
            if(absDy >= this.MIN_FIRE_OFFSET-7 && absDX <= 84) {
                if ((dx < 0 && targetTank.direction === this.DIRECTION.LEFT) || (dx > 0 && targetTank.direction === this.DIRECTION.RIGHT)) {
                    if (dy > 0) {
                        return this.DIRECTION.DOWN;
                    } else {
                        return this.DIRECTION.UP;
            ...
            if (absDX >= this.MIN_FIRE_OFFSET-7  && absDy <= 84) {
                if (dy < 0 && targetTank.direction === this.DIRECTION.UP || dy > 0 && targetTank.direction === this.DIRECTION.DOWN) {
                    if (dx > 0) {
                        return this.DIRECTION.LEFT;
                    } else {
                        return this.DIRECTION.RIGHT;
    ...
    isTargetDangerousDirection(targetTank, dx, dy) {
        switch(targetTank.direction) {
            case this.DIRECTION.UP:
                if (dy < 0 && Math.abs(dx) < 30) {
                    return true;
                } 
                break;
            case this.DIRECTION.DOWN:
    ...
    afterStepOne(dx, dy) {
        ...
        let targetTank = this.targetTankInfo[2];
        if (targetTank.X === 0) {
            if (targetTank.Y === 0 || targetTank.Y === canvas.height-this.TANK_WIDTH) {
                shouldFire = true;
            }
        }

        if (targetTank.X === canvas.width-this.TANK_WIDTH) {
            if (targetTank.Y === 0 || targetTank.Y === canvas.height-this.TANK_WIDTH) {
                shouldFire = true;
            }
        }

        if (!shouldFire && this.isTargetDangerousDirection(targetTank, dx, dy)) {
            // 两步非必杀的情况要判断敌方坦克是否转向了，如果转向了跟我方相对的方向，则开火
            // 敌方调转方向准备打我们，我们也开火
            shouldFire = true;
        }
        ...
        if (shouldFire) {
            if (absDX < 30 && absDy < 30) {
                // 两个方向都可打，则选择与当前方向相同的方向
                let direction1 = this.DIRECTION.STOP;
                let direction2 = this.DIRECTION.STOP;
                if (dx >= this.minAttackDistance) {
                    // 我方在右边
                    direction1 = this.DIRECTION.LEFT;
                } else {
                    // 我方在左边
                    direction1 = this.DIRECTION.RIGHT;
                }
    
                if (dy >= this.minAttackDistance) {
                    direction2 = this.DIRECTION.UP;
                } else {
                    direction2 = this.DIRECTION.DOWN;
                }
    
                if (this.svMyTank.direction === direction1) {
                    return [direction1, true];
                } else if (this.svMyTank.direction === direction2) {
        ...
        // TODO SCJ 判断对手方子弹数量及消失时间，决定是否要进入攻击范围
        // 对手方把子弹打完,更改最小距离
        if (this.enemyBullets.length === 5) {
            finalDirection = this.attack(dx, dy);
        } else {
            switch(targetTank.direction) {
                case this.DIRECTION.UP:
                    if (dy < 0) {
                        ...
                        finalDirection = this.pureAttack(dx, dy);
                    }
                    break;
                case this.DIRECTION.DOWN:
                    if (dy > 0) {
        ...
        return [finalDirection, shouldFire];
    }

    veryCautionAttack(dx, dy) {
