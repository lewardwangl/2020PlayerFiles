(function(type = "B", teamName = "红鲤鱼与绿鲤鱼与驴") {
  if (!["A", "B"].includes(type)) type = "B"; // exception
  // events:
  ...
  window[`player${type}`] = {
    land() {
      ...
      function avoidBullet(amSPIdFE1) {        for (const bullet of amSPIdFE1) {          const Ax = bullet["\x58"] - currentTank["\x58"] - 25;          const Ay = currentTank["\x59"] + 25 - bullet["\x59"];          const degree = direction2degree[bullet["\x64\x69\x72\x65\x63\x74\x69\x6f\x6e"]];          const Bx = zero(window["\x4d\x61\x74\x68"]["\x63\x6f\x73"]((degree / 180) * window["\x4d\x61\x74\x68"]["\x50\x49"]) * 1);          const By = zero(window["\x4d\x61\x74\x68"]["\x73\x69\x6e"]((degree / 180) * window["\x4d\x61\x74\x68"]["\x50\x49"]) * 1);          const innerDot = Ax * Bx + Ay * By;          const outerDot = Ax * By - Ay * Bx;          const distance = window["\x4d\x61\x74\x68"]["\x73\x71\x72\x74"](Ax * Ax + Ay * Ay);          const dangergap = window["\x4d\x61\x74\x68"]["\x6d\x69\x6e"](screenY / 5, screenX / 5);          if (innerDot < 0 && window["\x4d\x61\x74\x68"]["\x61\x62\x73"](outerDot) < 55 && distance < dangergap) {            let flag = window["\x4d\x61\x74\x68"]["\x73\x69\x67\x6e"](outerDot);            if(flag == 0) flag = window["\x4d\x61\x74\x68"]["\x72\x61\x6e\x64\x6f\x6d"]() > 0.5 ? 1 : -1;            let tempdir = flag * 90 + degree;            const tempdis = 40 / window["\x4d\x61\x74\x68"]["\x73\x71\x72\x74"](Ax * Ax + Ay * Ay);            const tempx = zero(window["\x4d\x61\x74\x68"]["\x63\x6f\x73"]((tempdir / 180) * window["\x4d\x61\x74\x68"]["\x50\x49"]));            const tempy = zero(window["\x4d\x61\x74\x68"]["\x73\x69\x6e"]((tempdir / 180) * window["\x4d\x61\x74\x68"]["\x50\x49"]));            moveX += tempx * tempdis;            moveY += tempy * tempdis;            console["\x77\x61\x72\x6e"](" \x69\x6e\x66\x6f", tempdir, moveX, moveY);            isDanger = true;          }        }      }
      ...
      if (isDanger) {        moveDirection = getTankDirection(currentTank, moveX, moveY);      } else {        let mindis = Infinity;        let mindistank = enemyTanks[0];        for (const enemyTank of enemyTanks) {          if (enemyTank === undefined) continue;          const curdis = getTankDistance(currentTank, enemyTank);          if (curdis < mindis) {            mindis = curdis;            mindistank = enemyTank;          }        }        const TAx = mindistank["\x58"] - currentTank["\x58"];        const TAy = currentTank["\x59"] - mindistank["\x59"];        const dist = window["\x4d\x61\x74\x68"]["\x73\x71\x72\x74"](TAx * TAx + TAy * TAy);        let tdegree = (window["\x4d\x61\x74\x68"]["\x61\x63\x6f\x73"](TAx / dist) * 180) / window["\x4d\x61\x74\x68"]["\x50\x49"];        if (TAy < 0) {          tdegree = 360 - tdegree;        }        let isAway = false;        if (dist < 60) {          tdegree += 180;          isAway = false;        }        const dir = getDegree2Dir(tdegree);        if (dir["\x6c\x65\x6e\x67\x74\x68"] > 0) {          if (dir["\x6c\x65\x6e\x67\x74\x68"] == 1) moveDirection = dir[0];          else {            const isOutX = checkOutOfScreen(currentTank, dir[0], ametal);            const isOutY = checkOutOfScreen(currentTank, dir[1], ametal);            if (isOutX || isOutY) {              moveDirection = isOutX ? dir[1] : dir[0];            } else {              if (isAway && enemyTanks["\x6c\x65\x6e\x67\x74\x68"] > 3) {                moveDirection = window["\x4d\x61\x74\x68"]["\x61\x62\x73"](TAx) > window["\x4d\x61\x74\x68"]["\x61\x62\x73"](TAy) ? dir[1] : dir[0];              } else {                moveDirection = window["\x4d\x61\x74\x68"]["\x61\x62\x73"](TAx) > window["\x4d\x61\x74\x68"]["\x61\x62\x73"](TAy) ? dir[0] : dir[1];              }            }          }        }
