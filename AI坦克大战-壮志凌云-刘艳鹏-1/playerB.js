window.playerB = new (class PlayerControl {
  ...
  land() {
    ...
    var lateEnemy = undefined                     //最近的敌方坦克
    var misDistanceOfEnemy = currentTankWH * 100  //与敌方坦克的最大临界距离
    var secruitydistance = currentTankWH * 6      //安全距离，
    var secruitylevel = enemyTanks.length         //安全等级，
    var firedirectdis = 3                         //正面开火距离
    var escapedir = 3                             //正面逃跑指数
    var fight = 4                                 //战斗指数
    var escapenum = 0                             //逃跑系数（周围敌方坦克数）

    //根据我方坦克和各个AI Tank的距离获取逃亡系数（escapeNum），并获取当前距离最近的AI Tank
    ...
    //   }
    // }
    //在两个玩家都在的情况下，根据设定的securityLevel等级判断是否加速打AI Tank
    ...
    if (moveDirection == undefined && escapenum < 4) {
      ...
      if (undefined != lateEnemy) {
        ...
        if ((disX > fight * currentTankWH || disY > fight * currentTankWH) && dis > fight * currentTankWH) {//追击
          ...
          if ((disX < disY) && (lateEnemy.Y < currentTankY) && this.#DIRECTION.RIGHT != Bullet[9] && this.#DIRECTION.STOP == Bullet[10] && this.#DIRECTION.LEFT != Bullet[11]) {
            moveDirection = this.#DIRECTION.DOWN;
          } else if ((disX < disY) && (lateEnemy.Y >= currentTankY) && this.#DIRECTION.RIGHT != Bullet[1] && this.#DIRECTION.STOP == Bullet[2] && this.#DIRECTION.LEFT != Bullet[3]) {
            moveDirection = this.#DIRECTION.UP;
          } else if ((disX > disY) && (lateEnemy.X >= currentTankX) && this.#DIRECTION.DOWN != Bullet[1] && this.#DIRECTION.STOP == Bullet[5] && this.#DIRECTION.UP != Bullet[9]) {
            moveDirection = this.#DIRECTION.LEFT;
          } else if ((disX > disY) && (lateEnemy.X < currentTankX) && this.#DIRECTION.DOWN != Bullet[3] && this.#DIRECTION.STOP == Bullet[7] && this.#DIRECTION.UP != Bullet[11]) {
            moveDirection = this.#DIRECTION.RIGHT
          }
      ...
      if (cy > currentTankY  && this.#DIRECTION.RIGHT != Bullet[9] && this.#DIRECTION.STOP == Bullet[10] && this.#DIRECTION.LEFT != Bullet[11]) {
        moveDirection = this.#DIRECTION.DOWN;
      } else if (cy > currentTankY && this.#DIRECTION.RIGHT != Bullet[1] && this.#DIRECTION.STOP == Bullet[2] && this.#DIRECTION.LEFT != Bullet[3]) {
        moveDirection = this.#DIRECTION.UP;
      } else if (cx < currentTankX && this.#DIRECTION.DOWN != Bullet[1] && this.#DIRECTION.STOP == Bullet[5] && this.#DIRECTION.UP != Bullet[9]) {
        moveDirection = this.#DIRECTION.LEFT;
      } else if (cx > currentTankX && this.#DIRECTION.DOWN != Bullet[3] && this.#DIRECTION.STOP == Bullet[7] && this.#DIRECTION.UP != Bullet[11]) {
        moveDirection = this.#DIRECTION.RIGHT
      }
  ...
  #avoidBullet(currentTankX, currentTankY, currentTankWH, Bullet, moveDirection) {
    ...
    */
    if (this.#DIRECTION.DOWN == Bullet[2] || this.#DIRECTION.UP == Bullet[10]) { //必须左右移动
      ...
      ) && this.#DIRECTION.DOWN != Bullet[3] && this.#DIRECTION.STOP == Bullet[7] && this.#DIRECTION.LEFT != Bullet[10] && this.#DIRECTION.UP != Bullet[11]) {
