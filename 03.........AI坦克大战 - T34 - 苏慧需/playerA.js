window.playerA = new (class PlayerControl {
    ...
    land() {
        ...
        if (moveDirection == undefined && escapenum < 4) {
            ...
            if (undefined != lateEnemy) {
                ...
                // console.log("kkkkkai huo",this.lastDir)
                if (this.#canFire(currentTank.direction, currentTankX,currentTankY, enemyTanks)&&this.needFire){
        ...
        if (undefined != moveDirection) {
            ...
            if (Math.floor(Math.random()*10)>5){
                moveDirection = this.#fireDirection(currentTankX,currentTankY, enemyTanks)
                moveDirection = this.#avoidBullet(currentTankX, currentTankY, currentTankWH, Bullet, moveDirection)
                // console.log("no move",moveDirection)
                this.priority=moveDirection;
                this.lastDir=moveDirection;
                this.needFire = true
            }
        }
    ...
    leave() {
        ...
        this.#setName();
        if (this.#canFire(currentTankDirect, currentTankX,currentTankY, enemyTanks)){
            var c = (new Date()).valueOf()
            if (c - this.firetimestamp > 150) {
                this.firetimestamp = c
                this.#fire();
    ...
    */
    ...
    #canFire(direction, curX,curY, tanks){
        for(let i = 0,aTankLen = tanks.length; i < aTankLen; i++){
            var dis = this.#calcTwoPointDistance(curX,curY,tanks[i]['X'],tanks[i]['Y'])
            // var rangeNum = (dis/200)*5 + 30 // 一定范围内由坦克就可以开火
            var rangeNum = 30 // 一定范围内由坦克就可以开火
            if (direction==this.#DIRECTION.UP){
                if (Math.abs(curX-tanks[i]['X'])<rangeNum && curY >= tanks[i]['Y']){
                    return true
                }
                ...
                if (Math.abs(curX-tanks[i]['Y'])<rangeNum && curX >= tanks[i]['X']){
                    return true
                }
    ...
    #fireDirection(currentTankX,currentTankY, enemyTanks){
        var lateEnemy = undefined
        var misDistanceOfEnemy = 50 * 100
        for (const enemy of enemyTanks) {
            const dis = this.#calcTwoPointDistance(
                currentTankX,
                currentTankY,
        ...
        var xDis = currentTankX-lateEnemy.X
        var yDis = currentTankY-lateEnemy.Y
