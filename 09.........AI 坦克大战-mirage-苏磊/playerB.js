window.playerB = new (class PlayerControl {
  ...
  land() {
    ...
    var secruitydistance = currentTankWH * 3
    ...
    if (moveDirection == undefined && escapenum < 4) {
      ...
      if (undefined != lateEnemy) {
        ...
        if (Bullet[2]==this.#DIRECTION.STOP && Bullet[5]==this.#DIRECTION.STOP &&Bullet[7]==this.#DIRECTION.STOP &&Bullet[10]==this.#DIRECTION.STOP && dis >= firedirectdis * currentTankWH) {//调整炮口
        ...
        if (Bullet[2]==this.#DIRECTION.STOP && Bullet[5]==this.#DIRECTION.STOP &&Bullet[7]==this.#DIRECTION.STOP &&Bullet[10]==this.#DIRECTION.STOP && dis > fight * currentTankWH) {//追击
          if ((disX < disY) && (lateEnemy.Y < currentTankY) && this.#DIRECTION.STOP == Bullet[1] && this.#DIRECTION.STOP == Bullet[2] && this.#DIRECTION.STOP == Bullet[3]) {
            moveDirection = this.#DIRECTION.UP;
          } else if ((disX < disY) && (lateEnemy.Y >= currentTankY) && this.#DIRECTION.STOP == Bullet[9] && this.#DIRECTION.STOP == Bullet[10] && this.#DIRECTION.STOP == Bullet[11]) {
            moveDirection = this.#DIRECTION.DOWN;
        ...
        else if (Bullet[2]==this.#DIRECTION.STOP && Bullet[5]==this.#DIRECTION.STOP &&Bullet[7]==this.#DIRECTION.STOP &&Bullet[10]==this.#DIRECTION.STOP && dis < escapedir * currentTankWH) {//逃跑
