window.playerA = new (class PlayerControl {
  ...
  land() {
    ...
    //中心区域方向
    const centerDirs = [];
    this.#initCenterDirs(myTank, cx, cy, tankWH, centerDirs);
    ...
    if(escapeDirs.length > 0){
      moveDirection = this.#avoidTank(myTank, centerDirs, escapeDirs, scanBullets, tankWH, bulletWH);
    }
    //追击敌方坦克
    if(moveDirection == undefined) {
      moveDirection = this.#attackTank(atkDis, myTank, latelyTank, tankWH, bulletWH, scanBullets, myAiTanks.length, centerDirs, enTank, enBullets);
    }
    //可以停留，直接开炮
    if(moveDirection == undefined) {
    ...
    if(moveDirection == undefined){
      ...
      for(let i=0; i<fireDirs.length; i++){
        ...
        if(moveDirection != undefined)break;
      }
    }
    //躲避子弹
    if(moveDirection == undefined){
      moveDirection = this.#avoidBullet(myTank, centerDirs, scanBullets, tankWH, bulletWH);
    }
    //开火
    if($.inArray(myTank.direction, fireDirs) >= 0){
  ...
  #canMoveToDir(tank, direction, bullets, tankWH, bulletWH) {
    //确定移动后坦克坐标
    let stepTank = this.#moveTank(tank.X, tank.Y, tank.speed, direction);
    //第一步移动
  ...
  #canMoveTank(tank, direction, bullets, tankWH, bulletWH, stepNum){
    ...
    for(let i=0; i<5; i++, sDir++) {
      let stepTank = this.#moveTank(tank.X, tank.Y, tank.speed, sDir % 5);
      if(this.#checkCollisionWithStep(stepTank, sDir % 5, bullets, stepNum, tankWH, bulletWH)){
        continue;
      }
   ...
   * @returns {Object}
   */
  #moveTank(tankX, tankY, tankSpeed, direction){
  ...
  //中心区域方向
  #initCenterDirs(myTank, centerX, centerY, tankWH, centerDirs){

    let disX = centerX - (myTank.X + tankWH/2);
    let disY = centerY - (myTank.Y + tankWH/2);
    ...
    if(Math.abs(disX) > centerX-tankWH && Math.abs(disY) > centerY-tankWH){
      if(centerX - Math.abs(disX) < centerY - Math.abs(disY)){
    ...
    }else if(Math.abs(disX) > centerX-tankWH){
      centerDirs.push(dirX);
    }else if(Math.abs(disY) > centerY-tankWH){
      centerDirs.push(dirY);
    }
  }

  //躲避子弹，优先中心区域
  #avoidBullet(myTank, centerDirs, bullets, tankWH, bulletWH){

    let moveDir = undefined;
    let dirs = $.extend(true, [], centerDirs);
    ...
    for(let dir of dirs){
      let canMove = this.#canMoveToDir(myTank, dir, bullets, tankWH, bulletWH);
      if(canMove){
        this.#move(dir);
        moveDir = dir;
        break;
  ...
  //躲避坦克，优先中心区域
  #avoidTank(myTank, centerDirs, escapeDirs, bullets, tankWH, bulletWH){

    let moveDir = undefined;

    let tmpCdir = $.extend(true, [], centerDirs);
    let tmpEdir = $.extend(true, [], escapeDirs);

    for(let dir of centerDirs){
      if($.inArray(dir, escapeDirs) > -1){
        this.#remove(dir, tmpEdir);
      }else {
        this.#remove(dir, tmpCdir);
      }
    }
    let dirs = tmpCdir.concat(tmpEdir);
    for(let dir of dirs){
      let canMove = this.#canMoveToDir(myTank, dir, bullets, tankWH, bulletWH);
      if(canMove){
        this.#move(dir);
  ...
  #myTankFire(myTank, aiTankNum, tankWH){
    ...
    if(aiTankNum >= 18){
      if(myTank.X < screenX/2 && myTank.X > 6*tankWH){
        return;
      }else if(myTank.X > screenX/2 && screenX - myTank.X > 7*tankWH){
        return;
      }
    }
  ...
  //追击坦克
  #attackTank(atkDis, myTank, latelyTank, tankWH, bulletWH, bullets, aiTankNum, centerDirs, enTank, enBullets){
    ...
    let moveDirArry = [];
    if(aiTankNum > 4 || atkDisX > atkDis*4){
      if(atkDisX > 20){
        if(myTank.X > latelyTank.X){
