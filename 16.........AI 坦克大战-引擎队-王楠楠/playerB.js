    ...
    if (limitDirect != 1) {
      if (moveDirection == this.#DIRECTION.STOP) {
        if (!(closetColRLDis == 10000 && closetColUDDis == 10000)) {
          ...
          if (MindisTank != undefined && MindisTank.id != 100) moveDirection = this.#getMaxDan_2(DanNearBa, Dan, 0.5)
          // 判断距离最近的坦克保持距离
          if (MindisTank != undefined && MindisTank.id != 100) {
            if (moveDirection == this.#DIRECTION.STOP) {
              if (MindisTank.id != 100) {
      ...
      case "save":
        if (CanAttack[currentTankDirect] != 0) {
          if (this.#IfCanAttack(
            ...
            lateEnemy.X,
            lateEnemy.Y
          ) && lateEnemy != undefined && lateEnemy.id == 100) {
            attack = 2
            console.log("----------------------------")
          }
